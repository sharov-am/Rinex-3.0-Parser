...
namespace Rinex3Parser.Nav
{
    ...
    public sealed class QzssNavMessage : NavMessage
    {
        #region Constructor

        internal QzssNavMessage(QzssSatellite satellitePrn, DateTime dt, [NotNull] double[] clkData,
        ...
        //sv, epoch, clk data
        public QzssSatellite SatellitePrn { get; private set; }
        ...
        {
            get { return SatelliteSystem.Qzss; }
        }
    ...
    public class QzssNavMessageContentParser : NavMessageContentParserBase<QzssNavMessage>
    {
        #region Constructor

        public QzssNavMessageContentParser(StreamReader sr)
                : base(sr, 8)
        {
        }
        ...
        protected override QzssNavMessage ParseContents([NotNull] string[] broadCastOrbits)
        {
            var svEpochClk = broadCastOrbits[0];
            QzssSatellite satellitePrn;
            DateTime dt;
            double[] clkData;
            ParseSvEpochClk(svEpochClk, out satellitePrn, out dt, out clkData);
            var orbitParameters = ParseContentsImpl(broadCastOrbits);
            return new QzssNavMessage(satellitePrn, dt, clkData, orbitParameters);
        }
