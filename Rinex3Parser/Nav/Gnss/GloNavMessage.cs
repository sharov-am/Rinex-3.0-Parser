...
namespace Rinex3Parser.Nav
{
    ...
    public class GloNavMessage : NavMessage
    {
        #region Constructor

        public GloNavMessage(GloSatellite satellitePrn, DateTime dt, double[] clkData, double[] orbitParameters)
        ...
        public double MessageFrameTime
        {
        ...
        public GloSatellite SatellitePrn { get; set; }

        public override SatelliteSystem SatelliteSystem
        {
            get { return SatelliteSystem.Glo; }
        }
    ...
    public class GloNavMessageContentParser : NavMessageContentParserBase<GloNavMessage>
    {
        #region Constructor

        public GloNavMessageContentParser(StreamReader sr)
                : base(sr, 4)
        {
        }
        ...
        protected override GloNavMessage ParseContents([NotNull]  string[] broadCastOrbits)
        {
            var svEpochClk = broadCastOrbits[0];
            GloSatellite satellitePrn;
            DateTime dt;
            double[] clkData;
            ParseSvEpochClk(svEpochClk, out satellitePrn, out dt, out clkData);
            var orbitParameters = ParseContentsImpl(broadCastOrbits);
            return new GloNavMessage(satellitePrn, dt, clkData, orbitParameters);
        }
